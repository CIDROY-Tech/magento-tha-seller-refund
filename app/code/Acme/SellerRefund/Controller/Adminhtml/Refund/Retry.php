...
{
    public const ADMIN_RESOURCE = 'Acme_SellerRefund::retry';
    ...
    public function execute(): Json
    {
        ...
        try {
            ...
            $this->processor->replay($refundId, $actorId);

            return $result->setData(['ok' => true, 'message' => (string) __('Refund operation queued for retry.')]);
        } catch (LocalizedException $e) {
            return $result->setData(['ok' => false, 'message' => $e->getMessage()]);
        } catch (\Throwable $e) {
            return $result->setData(['ok' => false, 'message' => (string) __('The refund could not be retried.')]);
        }
    }
}
